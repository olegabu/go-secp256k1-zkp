...
func TestAggsigGrin(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		R, err = AggsigGenerateSecureNonce(sign, nil)
		assert.NoError(t, err)
...
func TestAggsigGrinWithKernelOffset(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		// Randomize keys
		secBlinds[0], err = AggsigGenerateSecureNonce(sign, nil)
		assert.NoError(t, err)
		secBlinds[1], err = AggsigGenerateSecureNonce(sign, nil)
		assert.NoError(t, err)

		// Negate random offset from sender's blind 
		offset, err = AggsigGenerateSecureNonce(sign, nil)
		assert.NoError(t, err)
		rk
		sli ce, err = BlindSum(sign, [][32]byte{secBlinds[0]}, [][32]byte{offset})
		copy(secBlinds[0][:], slice)
		...
		// Combine pubnonces
		res, sumPubNonces, err := EcPubkeyCombine(ctx, []*PublicKey{pubNonces[0], pubNonces[1]})
		assert.True(t, res == 1)
		// Combine pubBlinds
		res, sumPubBlinds, err := EcPubkeyCombine(ctx, []*PublicKey{pubBlinds[0], pubBlinds[1]})
		assert.True(t, res == 1)

		msg32 := Random256()
		msg := msg32[:]

		// Create Receiver's partial Sig
		recvSig, err = AggsigSignSingle(
			sign,
			msg[:],
			secBlinds[1][:], secNonces[1][:], 
			nil, 
			sumPubNonces, sumPubNonces, sumPubBlinds, nil)
		assert.NoError(t, err)

		// Sender verifies Receiver's Sig then creates final combined Sig
		err = AggsigVerifySingle(
			vrfy,
			recvSig,
			msg[:],
			sumPubNonces, pubBlinds[1], sumPubBlinds,
			nil, true)
		assert.NoError(t, err)

		// Sender calculates it's signa1ture
		sendSig, err = AggsigSignSingle(
			sign, 
			msg[:], secBlinds[0][:], secNonces[0][:], nil, sumPubNonces, sumPubNonces, sumPubBlinds, nil)
		assert.NoError(t, err)
