...
const ErrorXonlyPubkeyTweakAdd = "error calling XonlyPubkeyTweakAdd"
const ErrorXonlyPubkeyTweakAddCheck = "error verifying XonlyPubkeyTweakAddCheck"
const ErrorKeypairCreate = "error calling KeypairCreate"
const ErrorKeypairPub = "error calling KeypairPub"
const ErrorKeypairXonlyPubKey = "error calling KeypairXonlyPubKey"
const ErrorKeypairXonlyTweakAdd = "error calling KeypairXonlyTweakAdd"
...
*/
func XonlyPubkeyParse(ctx *Context, input32 []byte) (pubkey *XonlyPubkey, err error) {
	var xopk XonlyPubkey
	if 1 != C.secp256k1_xonly_pubkey_parse(ctx.ctx, &xopk, cBuf(input32)) {
		err = errors.New(ErrorXonlyPubkeyParse)
	} else {
		pubkey = &xopk
	}
...
*/
func XonlyPubkeyFromPubkey(ctx *Context, pubkey *PublicKey) (xonly *XonlyPubkey, parity int, err error) {
	var xopk XonlyPubkey
	var par C.int
	if 1 != C.secp256k1_xonly_pubkey_from_pubkey(ctx.ctx, &xopk, &par, pubkey.pk) {
		err = errors.New(ErrorXonlyPubkeyFromPubkey)
	} else {
		xonly, parity = &xopk, int(par)
	}
...
*/
func XonlyPubkeyTweakAdd(ctx *Context, xonly *XonlyPubkey, tweak32 []byte) (pubkey *PublicKey, err error) {
	pubkey = newPublicKey()
	if 1 != C.secp256k1_xonly_pubkey_tweak_add(ctx.ctx, pubkey.pk, xonly, cBuf(tweak32)) {
		err = errors.New(ErrorXonlyPubkeyTweakAdd)
	}
...
*/
func XonlyPubkeyTweakAddCheck(ctx *Context, tweakedpubkey32 []byte, tweakedpubkeyparity int, internalpubkey *XonlyPubkey, tweak32 []byte) (err error) {
	if 1 != C.secp256k1_xonly_pubkey_tweak_add_check(ctx.ctx, cBuf(tweakedpubkey32), C.int(tweakedpubkeyparity), internalpubkey, cBuf(tweak32)) {
		err = errors.New(ErrorXonlyPubkeyTweakAddCheck)
	}
...
*/
func KeypairCreate(ctx *Context, seckey32 []byte) (keypair *Keypair, err error) {
	var kp Keypair
	if 1 != C.secp256k1_keypair_create(ctx.ctx, &kp, cBuf(seckey32)) {
		err = errors.New(ErrorKeypairCreate)
	} else {
		keypair = &kp
	}
...
*/
func KeypairPub(ctx *Context, keypair *Keypair) (pubkey *PublicKey, err error) {
	pubkey = newPublicKey()
	if 1 != C.secp256k1_keypair_pub(ctx.ctx, pubkey.pk, keypair) {
		err = errors.New(ErrorKeypairPub)
	}
...
*/
func KeypairXonlyPubkey(ctx *Context, keypair *Keypair) (xonly *XonlyPubkey, parity int, err error) {
	var xopk XonlyPubkey
	var par C.int
	if 1 != C.secp256k1_keypair_xonly_pub(ctx.ctx, &xopk, &par, keypair) {
		err = errors.New(ErrorKeypairXonlyPubKey)
	} else {
		xonly, parity = &xopk, int(par)
	}
...
*/
func KeypairXonlyTweakAdd(ctx *Context, keypair *Keypair, tweak32 []byte) (result *Keypair, err error) {
	var res Keypair = *keypair
	if 1 != C.secp256k1_keypair_xonly_tweak_add(ctx.ctx, &res, cBuf(tweak32)) {
		err = errors.New(ErrorKeypairXonlyTweakAdd)
	} else {
		result = &res
	}
