...
const (
	ErrorMusigPubkeyCombine             string = "error calling MusigPubkeyCombine"
	ErrorMusigPubkeyTweakAdd            string = "error calling MusigPubkeyTweakAdd"
...
func MusigPubkeyCombine(
	...
	var cpk XonlyPubkey
	var pre MusigPreSession

	pks := make([]XonlyPubkey, len(pubkeys))
	for i, pk := range pubkeys {
		pks[i] = *pk
	}

	if 1 != C.secp256k1_musig_pubkey_combine(
		ctx.ctx,
		scratch,
		&cpk,
		&pre,
		...
		err = errors.New(ErrorMusigPubkeyCombine)
	} else {
		combinedpubkey = &cpk
		presession = &pre
	}
...
func MusigPubkeyTweakAdd(
	...
	pubkey = newPublicKey()
	if 1 != C.secp256k1_musig_pubkey_tweak_add(
		ctx.ctx,
		presession,
		pubkey.pk,
		combinedpubkey,
		...
		err = errors.New(ErrorMusigPubkeyTweakAdd)
	}
...
func MusigSessionSetNonce(
	...
	if 1 != C.secp256k1_musig_set_nonce(
		ctx.ctx,
		signer,
		cBuf(nonce32),
	) {
...
func MusigPartialSignatureSerialize(
	...
	if 1 != C.secp256k1_musig_partial_signature_serialize(
		ctx.ctx,
		cBuf(out32[:]),
		sig,
	) {
		err = errors.New(ErrorMusigPartialSignatureSerialize)
	}
...
func MusigPartialSignatureParse(
	...
	if 1 != C.secp256k1_musig_partial_signature_parse(
		ctx.ctx,
		sig,
		cBuf(in32),
	) {
		err = errors.New(ErrorMusigPartialSignatureParse)
	}
...
func MusigPartialSign(
	...
	var sig MusigPartialSignature
	if 1 != C.secp256k1_musig_partial_sign(
		ctx.ctx,
		session,
		&sig,
	) {
		err = errors.New(ErrorMusigPartialSign)
	} else {
		partialsig = &sig
	}
...
func MusigPartialSigVerify(
	...
	if 1 != C.secp256k1_musig_partial_sig_verify(
		ctx.ctx,
		session,
		signer,
		partialsig,
		...
		err = errors.New(ErrorMusigPartialSigVerify)
	}
...
func MusigPartialSigCombine(
	...
	if 1 != C.secp256k1_musig_partial_sig_combine(
		ctx.ctx,
		session,
		cBuf(sig64[:]),
		sigs,
