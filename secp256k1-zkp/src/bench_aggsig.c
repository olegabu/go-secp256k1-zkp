...
void bench_aggsig_sign(void* arg) {
    ...
    for (i = 0; i < 1000; i++) {
        ...
        sk[0] = i;
        sk[1] = i >> 8;
        CHECK(secp256k1_aggsig_sign_single(data->ctx, &sig[0], NULL, msg, sk, NULL, NULL));
    }
}

void bench_aggsig_verify(void* arg, int iters) {
    bench_aggsig_data *data = (bench_aggsig_data *)arg;
    size_t i;
...
void bench_aggsig_verify_n(void* arg, int iters) {
    bench_aggsig_data *data = (bench_aggsig_data *)arg;
    size_t i, j;
...
int main(void) {
    ...
    data.sigs = (const unsigned char **)malloc(MAX_SIGS * sizeof(unsigned char *));

    for (i = 0; i < MAX_SIGS; i++) {
        unsigned char sk[32];
        unsigned char *msg = (unsigned char *)malloc(32);
        unsigned char *sig = (unsigned char *)malloc(sizeof(*sig));
