...
static int secp256k1_bulletproof_rangeproof_verify_impl(const secp256k1_context *ctx, secp256k1_scratch *scratch, const unsigned char* const* proof, const size_t n_proofs, const size_t plen, size_t nbits, const uint64_t* const* min_value, const secp256k1_ge* const* commitp, size_t n_commits, const secp256k1_ge *value_gen, const secp256k1_bulletproof_generators *gens, const unsigned char* const* extra_commit, size_t *extra_commit_len) {
    ...
    if (!secp256k1_scratch_max_allocation(&ctx->error_callback, scratch, n_proofs * (sizeof(*ecmult_data) + sizeof(*innp_ctx)) * 2)) {
        return 0;
    }
    const size_t scratch_checkpoint = secp256k1_scratch_checkpoint(&ctx->error_callback, scratch);

    ecmult_data = (secp256k1_bulletproof_vfy_ecmult_context *)secp256k1_scratch_alloc(&ctx->error_callback, scratch, n_proofs * sizeof(*ecmult_data));
    innp_ctx = (secp256k1_bulletproof_innerproduct_context *)secp256k1_scratch_alloc(&ctx->error_callback, scratch, n_proofs * sizeof(*innp_ctx));
