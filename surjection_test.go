...
func TestCommitmentAPI(t *testing.T) {
    ...
    var fixedInputTags [10]FixedAssetTag
    var fixedOutputTag FixedAssetTag
    var ephemeralInputTags [10]Generator
    var ephemeralOutputTag Generator
    var inputBlindingKeys [10][32]byte
    var outputBlindingKey [32]byte
    var serializedProof [SurjectionProofSerializationBytesMax]byte
    var serializedLen int
    var proof Surjectionproof
    var proofOnHeap *Surjectionproof
    var nInputs int = len(fixedInputTags) / len(fixedInputTags[0])
    var inputIndex int
    var eCount int
    var i int

    var seed [32]byte = Random256()
    1
    /* secp256k1_context_set_error_callback(none, counting_illegal_callback_fn, &ecount);
        ...
        comHex := comNone.Hex(ctx)
        comUnhex, err := ctxNone.CommitmentFromHex(comHex)
        assert.NoError(t, err)
