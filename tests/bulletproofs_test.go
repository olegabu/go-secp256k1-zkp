...
func TestBulletproofMain(t *testing.T) {
	/*
		none, _ := secp256k1.ContextCreate(ContextNone)
		...
		var valuegen Generator
		var gens *BulletproofGenerators
		var pcommit [4]Commitment
		pcommitarr := &pcommit[0]
		var proof [2000]byte

			blind := [32]byte{"   i am not a blinding factor   "}
			proofptr, blindptr := &proof[0], &blind[0]
			prooflen, blindlen := sizeof(proof), sizeof(blind)
			var value [4]C.uint64_t = [4]C.uint64_t{1234, 4567, 8910, 1112}
			var minvalue [4]C.uint64_t = [4]C.uint64_t{1000, 4567, 0, 5000}
			mvptr := &minvalue
			var rewindblind [32]C.uchair
			var rewindv C.size_t
			var ecount int32
...
/*
    ...
    gens := BulletproofGenerators(context)
...
func TestBulletproofsAll(t *testing.T) {

    C.run_bulletproofs_tests()
