...
func TestGeneratorAPI(t *testing.T) {
	...
	ctxNone, err := secp256k1.ContextCreate(secp256k1.ContextNone)
	assert.NoError(t, err)
	assert.NotNil(t, ctxNone)
	assert.IsType(t, secp256k1.Context{}, *ctxNone)

	ctxSign, err := secp256k1.ContextCreate(secp256k1.ContextSign)
	assert.NoError(t, err)
	assert.NotNil(t, ctxSign)
	assert.IsType(t, secp256k1.Context{}, *ctxSign)

	ctxVrfy, err := secp256k1.ContextCreate(secp256k1.ContextVerify)
	assert.NoError(t, err)
	assert.NotNil(t, ctxVrfy)
	assert.IsType(t, secp256k1.Context{}, *ctxVrfy)

	ctxBoth, err := secp256k1.ContextCreate(secp256k1.ContextVerify | secp256k1.ContextSign)
	assert.NoError(t, err)
	assert.NotNil(t, ctxBoth)
	assert.IsType(t, secp256k1.Context{}, *ctxBoth)
	...
	fmt.Printf("genSignBlindedParsed=%v\n", *genSignBlindedParsed)

	secp256k1.ContextDestroy(ctxBoth)
	secp256k1.ContextDestroy(ctxVrfy)
	secp256k1.ContextDestroy(ctxSign)
	secp256k1.ContextDestroy(ctxNone)
}
