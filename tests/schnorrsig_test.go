...
func init() {
	var err error
	ctx, err = zkp.ContextCreate(zkp.ContextSign | zkp.ContextVerify)
	if err != nil {
		panic(err)
	}
}
...
// }
func TestSchnorrsigSerialize(t *testing.T) {
...
func TestSchnorrsigBIPVectors(t *testing.T) {
	scratch, err := zkp.ScratchSpaceCreate(ctx, 1024*1024)
	assert.NoError(t, err)
	testSchnorrsigBIPVectors(t, scratch)
	zkp.ScratchSpaceDestroy(scratch)
}

// // Helper function for schnorrsig_bip_vectors
// // Signs the message and checks that it's the same as expected_sig. //
// void test_schnorrsig_bip_vectors_check_signing(const unsigned char *sk, const unsigned char *pk_serialized,
//     const unsigned char *msg, const unsigned char *expected_sig, const int expected_nonce_is_negated) {
//     secp256k1_schnorrsig sig;
...
// }
func testSchnorrsigBIPVectorsCheckSign(
	t *testing.T,
	seckey [32]byte, pubkeyserialized []byte, message [32]byte, expectsig []byte,
	expectnoncenegated int,
) {
	sig, nin, err := zkp.SchnorrsigSign(ctx, message, seckey)
	assert.NoError(t, err)
	assert.NotNil(t, sig)
	assert.Equal(t, expectnoncenegated == 1, nin)

	sigser, err := zkp.SchnorrsigSerialize(ctx, sig)
	assert.NoError(t, err)
	assert.NotNil(t, sigser)
	...
	err = zkp.SchnorrsigVerify(ctx, sig, message[:], pubkey)
	assert.NoError(t, err)
}

// // Checks that both verify and verify_batch return the same value as expected. //
// void test_schnorrsig_bip_vectors_check_verify(secp256k1_scratch_space *scratch, const unsigned char *pk_serialized,
//     const unsigned char *msg32, const unsigned char *sig_serialized, int expected) {
...
// }
func testSchnorrsigBIPVectorsCheckVerify(
	t *testing.T,
	scratch *zkp.ScratchSpace,
	pubkeyserialized []byte, message [32]byte, sigserialized [64]byte,
	expectresult int,
) {
	successint, pubkey, err := zkp.EcPubkeyParse(ctx, pubkeyserialized)
	assert.True(t, successint == 1)
	...
	sig, err := zkp.SchnorrsigParse(ctx, sigserialized[:])
	assert.NoError(t, err)
	assert.NotNil(t, sig)

	err = zkp.SchnorrsigVerify(ctx, sig, message[:], pubkey)
	assert.Equal(t, expectresult == 1, err == nil)

	sigs := make([]*zkp.Schnorrsig, 1)
	sigs[0] = sig
	msgs := make([][32]byte, 1)
	copy(msgs[0][:], message[:])
	pubs := make([]*zkp.PublicKey, 1)
	pubs[0] = pubkey

	err = zkp.SchnorrsigVerifyBatch(ctx, scratch, sigs, msgs, pubs)
	assert.Equal(t, expectresult == 1, err == nil)
}
...
func testSchnorrsigBIPVectors(t *testing.T, scratch *zkp.ScratchSpace) {
	{
		// Test vector 1 //
		sk1 := [32]byte{
		...
		testSchnorrsigBIPVectorsCheckSign(t, sk1, pk1[:], msg1, sig1[:], 1)
		testSchnorrsigBIPVectorsCheckVerify(t, scratch, pk1[:], msg1, sig1, 1)
	}
	{
		// Test vector 2 //
		sk2 := [32]byte{
		...
		testSchnorrsigBIPVectorsCheckSign(t, sk2, pk2[:], msg2, sig2[:], 0)
		testSchnorrsigBIPVectorsCheckVerify(t, scratch, pk2[:], msg2, sig2, 1)
	}
	{
		// Test vector 3 //
		sk3 := [32]byte{
		...
		testSchnorrsigBIPVectorsCheckSign(t, sk3, pk3[:], msg3, sig3[:], 0)
		testSchnorrsigBIPVectorsCheckVerify(t, scratch, pk3[:], msg3, sig3, 1)
	}
	{
		// Test vector 4 //
		pk4 := [33]byte{
		...
		testSchnorrsigBIPVectorsCheckVerify(t, scratch, pk4[:], msg4, sig4, 1)
	}
	{
		// Test vector 5 //
		pk5 := [33]byte{
		...
		testSchnorrsigBIPVectorsCheckVerify(t, scratch, pk5[:], msg5, sig5, 1)
	}
	{
		// Test vector 6 //
		pk6 := [33]byte{
		...
		testSchnorrsigBIPVectorsCheckVerify(t, scratch, pk6[:], msg6, sig6, 1)
	}
	{
		// Test vector 7 //
		pk7 := [33]byte{
		...
		success, pk7parsed, err := zkp.EcPubkeyParse(ctx, pk7[:])
		assert.True(t, success == 0 || err != nil)
		assert.Nil(t, pk7parsed)
	...
	}
	{
		...
		testSchnorrsigBIPVectorsCheckVerify(t, scratch, pk8[:], msg8, sig8, 0)
	}
	{
		// Test vector 9 //
		pk9 := [33]byte{
		...
		testSchnorrsigBIPVectorsCheckVerify(t, scratch, pk9[:], msg9, sig9, 0)
	}
	{
		// Test vector 10 //
		pk10 := [33]byte{
		...
		testSchnorrsigBIPVectorsCheckVerify(t, scratch, pk10[:], msg10, sig10, 0)
	}
	{
		// Test vector 11 //
		pk11 := [33]byte{
		...
		testSchnorrsigBIPVectorsCheckVerify(t, scratch, pk11[:], msg11, sig11, 0)
	}
	{
		// Test vector 12 //
		pk12 := [33]byte{
		...
		testSchnorrsigBIPVectorsCheckVerify(t, scratch, pk12[:], msg12, sig12, 0)
	}
	{
		// Test vector 13 //
		pk13 := [33]byte{
		...
		testSchnorrsigBIPVectorsCheckVerify(t, scratch, pk13[:], msg13, sig13, 0)
	}
	{
		// Test vector 14 //
		pk14 := [33]byte{
		...
		testSchnorrsigBIPVectorsCheckVerify(t, scratch, pk14[:], msg14, sig14, 0)
	}
	{
		// Test vector 15 //
		pk15 := [33]byte{
		...
		testSchnorrsigBIPVectorsCheckVerify(t, scratch, pk15[:], msg15, sig15, 0)
	}
	{
		// Test vector 16 //
		pk16 := [33]byte{
		...
		testSchnorrsigBIPVectorsCheckVerify(t, scratch, pk16[:], msg16, sig16, 0)
	}
}
